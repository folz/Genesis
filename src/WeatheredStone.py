...
import Client
import NetworkBullets

id = int(input("ID (1 or 2): "))
player2 = None
...
def useData(data):
    ...
    if data.pId != id:
      #  print(data)
        player2.location = Vector(data.px,data.py)
...
world.setGravity(Vector(0, 1))
world.debug = True

global networkBullets
networkBullets = NetworkBullets.NetworkBulletManager(world)


def createBlock(x,y,w,h):   
    
    p = Terrain([(x,y),(x+w,y),(x+w,y+h),(x,y+h)],world)
    world.addTerrain(p)

def makeTerrain():
    leftWall = Slope([(0, 0), (0, 2000)])
    world.addTerrain(leftWall)
    rightWall = Slope([(2000, 0), (2000, 2000)])
    world.addTerrain(rightWall)
    topWall = Slope([(0, 0), (2000, 0)])
    world.addTerrain(topWall)
    bottomWall = createBlock(0, 2000, 2000, 50)
    #world.addTerrain(bottomWall)
    
    #top level
    p = Terrain([(1350,350),(1500,350),(1500,250)],world)
    world.addTerrain(p)
    p = Terrain([(700,200),(1050,350),(1050,400),(700,250)],world)
    world.addTerrain(p)
...
viewport = Viewport(window, world)

# create a player character
if id == 1: player = PlayerEntity("blue",(1940, 1940))
else: player = PlayerEntity("red",(100, 100))
world.addEntity(player)
player.addGun(BulletManager(player))

flag = FlagEntity("red", (50,175))
flag2 = FlagEntity("blue", (1950, 1975))
...
if id == 2: player2 = PlayerEntity("blue",(1940, 1940),"green-soldier.png")
else: player2 = PlayerEntity("red",(100, 100),"green-soldier.png")
world.addEntity(player2)
world.setPlayer2(player2)
player2.addGun(BulletManager(player2))
...
def sendData():
    ...
    if id == 1: 
        ...
        score = flag.score
        cap = flag2.captured
    data = Client.Data(player.location.x, player.location.y,bs,id,player2.hit,player.facing,flagFace,score,cap)
